...
if (mode === "youtube") {
    try {
        ...
        if (data.error) {
            return res.status(500).json({ error: data.error.message, debug: [JSON.stringify(data.error)] });
        }

        if (!data.items) {
            return res.status(200).json({ top: [], debug: ["data.items is empty"] });
        }
